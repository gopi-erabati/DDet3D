...
from ...core.bbox.util import (normalize_bbox, denormalize_bbox,
                               ...
                               normalize_0to1_bbox_bev,
                               denormalize_0to1_bbox_bev,
                               xyxyr2xywhr, xywhrr2xyxyrr,
...
class DynamicDDet3DHeadV2(BaseDenseHead):
    """
    ...
    def __init__(self,
        ...
        super(DynamicDDet3DHeadV2, self).__init__(init_cfg)
        ...
        if self.with_lidar_encoder:
            ...
            lidar_feat_lvls = (lidar_encoder_cfg.transformerlayers.attn_cfgs
                               .num_levels)
    ...
    def forward_train(self,
        ...
        prepare_targets = self.prepare_training_targets(gt_bboxes,
                                                        gt_labels, points,
                                                        epoch)
        (batch_gt_instances, batch_pred_instances) = prepare_targets
        # ([gt_inst, ...]: bboxes (n_gt, 9), labels (n_gt, ),
        # [pred_inst,...]: time, diff_bboxes (n_p, 6) in XYWHR,
        #                   diff_bboxes_abs (n_p, 5) in XYWHR,
        #                   noise (n_p, 6) XYWHRR)
        ...
        # batch_diff_bboxes = torch.stack([torch.cat([gt_instance.bboxes[
        # ..., :6
        #                                             ], gt_instance.bboxes[
    ...
    def forward(self, img_feats, point_feats, bboxes_init, time_init,
        ...
        bboxes_init (Tensor): (bs, n_p, 5) in XYWHR,
        time_init (Tensor): (bs, )
        ...
        for head_idx, single_head in enumerate(self.head_series):
            ...
            bboxes_recurr_3d = pred_bboxes.clone().detach()
            bboxes = pred_bboxes.clone().detach()[..., [0, 1, 3, 4, 6, 7]]
            bboxes[..., 2:4] = bboxes[..., 2:4].exp()
            rot = torch.atan2(bboxes[..., 4:5], bboxes[..., 5:6])  # ry
            bboxes = torch.cat([bboxes[..., :4], rot], dim=-1).to(
                bboxes.device)  # (n_p, 5) XYWHR
    ...
    def prepare_training_targets(self, gt_bboxes, gt_labels, points_list,
                                 epoch=None):
        ...
        for gt_bbox, gt_label, points in zip(gt_bboxes, gt_labels,
                                             points_list):
            ...
            gt_bbox_tensor = torch.cat((gt_bbox.gravity_center,
                                        gt_bbox.tensor[:,
                                        3:]), dim=1).to(
                device)
            gt_instances.bboxes = gt_bbox_tensor  # (n_gt, 9)
            gt_instances.labels = gt_label  # (n_gt, )

            # get the BEV box of GT
            gt_bbox_bev = gt_bbox.bev.to(device)  # (n_gt, 5) XYWHR

            # convert XYWHR to XYXYR
            gt_bbox_bev_xyxyr = xywhr2xyxyr(gt_bbox_bev)  # (n_gt, 5) XYXYR

            # Normalize 0 to 1 gt_bbox_bev_xyxyr
            gt_bbox_bev_xyxyr_norm = normalize_0to1_bbox_bev(
                gt_bbox_bev_xyxyr, self.pc_range)  # (n_gt, 6) XYXYRR

            # convert back XYXYRR to XYWHRR
            gt_bbox_bev_norm = xyxyr2xywhr(gt_bbox_bev_xyxyr_norm)
            # (n_gt, 6) in XYWHRR

            pred_instances = self.prepare_diffusion(gt_bbox_bev_norm,
                                                    points, epoch)
            gt_instances.norm_bbox = gt_bbox_bev_norm
    ...
    def prepare_diffusion(self, gt_boxes, points, epoch=None):
        """
        Prepare diffusion (noisy) gt boxes as initial bboxes
        gt_bboxes: (n_gt, 6) XYWHRR
        """
        device = gt_boxes.device

        # get time by epoch
        if epoch is None:
            epoch = 20  # to set timesteps w/o factor
        if epoch < 19:
            timestep_factor = np.sin((np.arccos(0.005) / 20) * epoch +
                                     np.arcsin(0.005))
            timesteps = int(self.timesteps * timestep_factor)
        else:
            timesteps = self.timesteps
        time = torch.randint(
            0, timesteps, (1,), dtype=torch.long, device=device)  # (1,)

        num_gt = gt_boxes.shape[0]
        if num_gt < self.num_proposals:
            # 3 * sigma = 1/2 --> sigma: 1/6
            # box_placeholder = torch.randn(
            #     self.num_proposals - num_gt, 6,
            #     device=device) / 6. + 0.5
            # # to make the values lie between 0 to 1 ; 3 sigma rule
            # box_placeholder[:, 2:4] = torch.clip(
            #     box_placeholder[:, 2:4], min=1e-4)
            # x_start = torch.cat((gt_boxes, box_placeholder), dim=0)
            # repeat the GT boxes to num of proposals
            num_gt_req = self.num_proposals - num_gt
            num_times_gt_repeat = num_gt_req // num_gt
            num_gt_repeat = num_gt_req % num_gt
            x_start = gt_boxes.repeat((num_times_gt_repeat + 1), 1)
            x_start = torch.cat((x_start, gt_boxes[:num_gt_repeat, :]),
                                dim=0)
        else:
            select_mask = [True] * self.num_proposals + \
                          [False] * (num_gt - self.num_proposals)
            random.shuffle(select_mask)
            x_start = gt_boxes[select_mask]  # (n_p, 6)

        def prep_diff(boxes):
            x_start_snr = (boxes * 2. - 1.) * self.snr_scale  # (n_p, 6)

            # noise sample
            noise = torch.randn(boxes.size(0), 6, device=device)
            # (n_p, 6)
            x = self.q_sample(x_start=x_start_snr, time=time, noise=noise)  # (
            # n_p, 6)

            x = torch.clamp(x, min=-1 * self.snr_scale, max=self.snr_scale)
            x = ((x / self.snr_scale) + 1) / 2.  # (n_p, 6)

            # change any size=0 to min value so that log wont get -inf
            # size = x[:, 3:6].clone()  # (n_p, 3)
            # size[size == 0.0] = 1e-4
            # x[:, 3:6] = size
            x[:, 2:4] = torch.clip(x[:, 2:4], min=1e-4)

            bboxes_diff = x
            # Convert boxes from XYWHRR to XYXYRR to denormalize them
            diff_bboxes_xyxyrr = xywhrr2xyxyrr(x)  # (n_p, 6) in XYXYRR

            # convert to abs bboxes
            diff_bboxes_xyxyr_abs = denormalize_0to1_bbox_bev(
                diff_bboxes_xyxyrr, self.pc_range)
            # (n_p, 5) in XYXYR

            # convert XYXYR to XYWHR
            bboxes_diff_abs = xyxyr2xywhr(diff_bboxes_xyxyr_abs)
            # (n_p, 5) in XYWHR

            return bboxes_diff_abs, bboxes_diff, noise

        # initial diff boxes
        diff_bboxes_abs, diff_bboxes, noise = prep_diff(x_start)

        # resample boxes with nm of points less than 5 points
        def check_diff_boxes_abs_resample(boxes_diff_abs):
            rbbox = torch.cat([boxes_diff_abs[..., 0:2],
                               boxes_diff_abs.new_ones(boxes_diff_abs.size(0),
                                1) * -3.0,
                               boxes_diff_abs[..., 2:4],
                               boxes_diff_abs.new_ones(boxes_diff_abs.size(0),
                                                        1) * 5.0,
                               boxes_diff_abs[..., 4:5],],
                              dim=-1)  # (n_p, 7)
            indices = box_np_ops.points_in_rbbox(points.cpu().numpy(),
                                                 rbbox.cpu().numpy())  #
            # (N, n_p)
            # it gives indices of boxes for each point
            num_points_in_diffboxes = indices.sum(0)  # (n_p, )
            diffbox_idx_resamp = num_points_in_diffboxes < 5
            return diffbox_idx_resamp

        diffbox_idx_resample = check_diff_boxes_abs_resample(diff_bboxes_abs)
        # (n_p, )

        if diffbox_idx_resample.any():
            boxes_resamp_start = x_start[diffbox_idx_resample]
            diff_bboxes_abs_resamp, diff_bboxes_resamp, noise_resamp = (
                prep_diff(boxes_resamp_start))

            # check any diff boxes < 5 points again
            diffbox_idx_resample_1 = check_diff_boxes_abs_resample(
                diff_bboxes_abs_resamp)

            # get nonzero of diffbox_idx_resample
            diffbox_idx_resample_nonz = diffbox_idx_resample.nonzero()[0]  # (n_p_nonzero, )
            diff_bboxes_abs[diffbox_idx_resample_nonz[~diffbox_idx_resample_1]] = (
                diff_bboxes_abs_resamp[~diffbox_idx_resample_1])
            diff_bboxes[diffbox_idx_resample_nonz[~diffbox_idx_resample_1]] = diff_bboxes_resamp[~diffbox_idx_resample_1]
            noise[diffbox_idx_resample_nonz[~diffbox_idx_resample_1]] = noise_resamp[~diffbox_idx_resample_1]

            diffbox_idx_resample[diffbox_idx_resample_nonz[
                ~diffbox_idx_resample_1]] = diffbox_idx_resample_1[~diffbox_idx_resample_1]

        # correlation on size w = rho * l + sqrt(1-rho**2)x
        rho = 0.8
        boxes_l = torch.randn(self.num_proposals, 1, device=device) / 6. + 0.5
        boxes_l = torch.clip(boxes_l, min=1e-4)
        boxes_x = torch.randn(self.num_proposals, 1, device=device) / 6. + 0.5
        boxes_x = torch.clip(boxes_x, min=1e-4)
        boxes_w = rho * boxes_l + np.sqrt((1 - rho**2)) * boxes_x
        boxes_w = boxes_w * 8.0  # (n_p, 1)
        boxes_l = boxes_l * 5.0  # (n_p, 1)

        diff_bboxes_abs[..., 2:3] = boxes_w
        diff_bboxes_abs[..., 3:4] = boxes_l

        metainfo = dict(time=time.squeeze(-1))
        pred_instances = InstanceData(metainfo=metainfo)
        pred_instances.diff_bboxes = diff_bboxes  # (n_p, 6) in XYWHRR
        pred_instances.diff_bboxes_abs = diff_bboxes_abs  # (n_p, 5) in XYWHR
    ...
    def q_sample(self, x_start, time, noise=None):
        ...
        x_start_shape = x_start.shape  # (n_p, 6)
        ...
        return sqrt_alphas_cumprod_t * x_start + \
            sqrt_one_minus_alphas_cumprod_t * noise  # (n_p, 6)
    ...
    def loss_classification(self, outputs, batch_gt_instances, indices):
        ...
        Args:
            ...
            indices (list[Tensor]): list[(n_p, )(n_p_gt), ()(), ...bs]
                    fg_mask_inboxes: gives pred indices where it has one
                    matched gt
                    matched_gt_inds: gives the indices of matched gt for
                    each pred box
        """
    ...
    def simple_test_bboxes(self, img_feats, point_feats, img_metas):
        ...
        (time_pairs, batch_noise_bboxes, batch_noise_bboxes_raw) = \
            self.prepare_testing_targets(
                img_metas, device)
        # [(999, -1)] (samp_ts+1, ), (bs, n_p, 5) in XYWHR, (bs, n_p,
        # 6) in XYWHRR
    ...
    def prepare_testing_targets(self, img_metas, device):
        ...
        for _ in range(len(img_metas)):
            noise_bboxes_raw = torch.randn(
                (self.num_proposals, 6),
                device=device)  # (n_p, 6) in XYWHRR
            ...
            noise_bboxes[:, 2:4] = torch.clip(noise_bboxes[:, 2:4], min=1e-4)

            # convert XYWHRR to XYXYRR
            noise_bboxes_xyxyr = xywhrr2xyxyrr(noise_bboxes)

            # convert to abs bboxes
            noise_bboxes_xyxyr_abs = denormalize_0to1_bbox_bev(
                noise_bboxes_xyxyr, self.pc_range)
            # (n_p, 5) in XYXYR

            # convert XYXYR to XYWHR
            noise_bboxes_abs = xyxyr2xywhr(noise_bboxes_xyxyr_abs)
            # (n_p, 5) in XYWHR

            # correlation on size w = rho * l + sqrt(1-rho**2)x
            rho = 0.8
            boxes_l = torch.randn(self.num_proposals, 1,
                                  device=device) / 6. + 0.5
            boxes_l = torch.clip(boxes_l, min=1e-4)
            boxes_x = torch.randn(self.num_proposals, 1,
                                  device=device) / 6. + 0.5
            boxes_x = torch.clip(boxes_x, min=1e-4)
            boxes_w = rho * boxes_l + np.sqrt((1 - rho ** 2)) * boxes_x
            boxes_w = boxes_w * 8.0  # (n_p, 1)
            ...
            noise_bboxes_abs[..., 2:3] = boxes_w
            noise_bboxes_abs[..., 3:4] = boxes_l

            noise_bboxes_raw_list.append(noise_bboxes_raw)
            noise_bboxes_list.append(noise_bboxes_abs)

        batch_noise_bboxes = torch.stack(noise_bboxes_list)  # (bs, n_p, 5)
    ...
    def predict_by_feat(self,
        ...
        Args:
            ...
            batch_noise_bboxes (Tensor): initial noise boxes of shape
                (bs, n_p, 5) in XYWHR in world coords
            batch_noise_bboxes_raw (Tensor): initial noise boxes of shape
                (bs, n_p, 6) in XYWHRR in raw randn format
        ...
        for time, time_next in time_pairs:  # 999, -1
            ...
            # if we use only one sampling step then we will continue and do
            # inference at last
            ...
            # between [0, 1]
            x_start_bbox = denormalize_bbox(x_start, self.pc_range,
                                            is_height_norm=False)
            # the above gives (bs, n_p, 7) with [xyzwhlr]
            # get the BEV type box for next step
            x_start_bbox_bev = x_start_bbox[..., [0, 1, 3, 4, 6]]
            # (bs, n_p, 5) in XYWHR
            # convert XYWHR to XYXYR to norm them
            x_start_bbox_bev_xyxyr = xywhr2xyxyr(
                x_start_bbox_bev)  # (bs, n_p, 5) XYXYR

            # Normalize 0 to 1 gt_bbox_bev_xyxyr
            x_start_bbox_bev_xyxyr_norm = normalize_0to1_bbox_bev(
                x_start_bbox_bev_xyxyr, self.pc_range)  # (bs, n_p, 6) XYXYRR

            x_start = xyxyr2xywhr(x_start_bbox_bev_xyxyr_norm)
            # (bs, n_p, 6) XYWHRR
            ...
            pred_noise_list = []  # This contains pred noise for boxes > thresh
            ...
            if self.box_renewal:  # filter
                ...
                for batch_id in range(batch_size):
                    ...
                    noise_bboxes_list.append(batch_noise_bboxes[batch_id,
                                             keep_idx, :])
                    # pred_noise and x_start contains (n_p_th, 6) in XYWHRR
                    # noise_bboxes contain (n_p_th, 5) in XYWHR
            else:
                for batch_id in range(batch_size):
            ...
            # drop_bboxes_vis[:, 2:4] = torch.clip(drop_bboxes_vis[:, 2:4],
            # min=1e-4)
            #
            ...
            #
            # drop_bboxes_vis = torch.cat([drop_bboxes_vis[..., :6], rot],
            # dim=1)
            #
            ...
            for idx in range(batch_size):
                pred_noise = pred_noise_list[idx]  # (n_p_th, 6) XYWHRR
                x_start = x_start_list[idx]  # (n_p_th, 6) XYWHRR
                noise_bboxes = noise_bboxes_list[idx]  # (n_p_th, 5) XYWHR
                num_remain = num_remain_list[idx]  # num
                noise = torch.randn_like(x_start)  # (n_p_th, 6)
                ...
                #     max=self.snr_scale)
                # ddim_bboxes_vis = ((ddim_bboxes_vis / self.snr_scale) + 1)
                # / 2
                #
                # # change any size=0 to min value so that log wont get -inf
                ...
                # ddim_bboxes_vis[:, 2:4] = torch.clip(ddim_bboxes_vis[:,
                # 2:4], min=1e-4)
                #
                ...
                if self.box_renewal:  # filter
                    # replenish with randn boxes or repeat boxes instead
                    if num_remain < self.num_proposals:
                        ...
                        #          device=device)),
                        #     dim=0)  # (n_p, 6)
                        num_prop_req = self.num_proposals - num_remain
                        num_times_prop_repeat = num_prop_req // num_remain
                        num_prop_repeat = num_prop_req % num_remain
                        noise_bboxes = noise_bboxes.repeat((
                                num_times_prop_repeat + 1), 1)
                        noise_bboxes = torch.cat(
                            (noise_bboxes, noise_bboxes[:num_prop_repeat, :]),
                            dim=0)  # (n_p, 6) XYWHRR
                    else:
                        select_mask = [True] * self.num_proposals + \
                                      [False] * (num_remain -
                                                 self.num_proposals)
                        random.shuffle(select_mask)
                        noise_bboxes = noise_bboxes[select_mask]  # (n_p, 6)
                ...
                noise_bboxes[:, 2:4] = torch.clip(noise_bboxes[:, 2:4],
                                                  min=1e-4)
                ...
                batch_noise_bboxes_list.append(noise_bboxes_abs)
            ...
            for idx in range(batch_size):
                ...
                if self.use_nms:
                    ...
                    for i, (scores_per_sample_step,
                        ...
                        scores_per_sample_step, topk_indices = (
                            scores_per_sample_step.flatten(
                                0, 1).topk(cfg.max_per_img))  # (n_max, )
                        labels_per_sample_step = (topk_indices %
                                                  self.num_classes)  # (
                        # n_max, )
                        ...
                        # contains (list[Tensor]) for steps ; (n_max,
                        # 9) (n_max, )
        ...
        if self.use_focal_loss or self.use_fed_loss:
            ...
            for i, (scores_per_sample,
                ...
                if self.use_nms:
                    ...
                    scores_per_sample, topk_indices = (
                        scores_per_sample.flatten(
...
@HEADS.register_module()
class SingleDDet3DHeadV2(BaseModule):
    """
    ...
    def __init__(self,
                 ...
                 voxel_size=None,
                 init_cfg=None):
        super(SingleDDet3DHeadV2, self).__init__(init_cfg)
        ...
        #     self.init_cfg = [
        #         dict(
        #             type='Normal', std=0.01, override=dict(
        #             name='class_logits')),
        #         dict(
        #             type='Normal', std=0.001, override=dict(
        #             name='bboxes_delta'))
        #     ]
    ...
    def forward(self, img_feats, point_feats, bboxes, prop_feats, pooler,
                time_emb, img_metas, pooler_img=None, bboxes_recurr_3d=None,
                head_idx=None):
        """
        img_feats (list[Tensor]): shape (bs, n_cam, C, H, W)
        point_feats (list[Tensor]): shape (bs, C, H, W)
        bboxes (Tensor): (bs, n_p, 5)
        prop_feats (Tensor|None)
        pooler (nn.Module)
        time_emb (Tensor): (bs, 256*4)
        img_metas (list[dict])

        bboxes_recurr_3d : (bs*n_p, 8) xyzwhlsincos
        head_idx
        """
        ...
        if head_idx == 0:
            ...
            pred_bboxes = self.apply_deltas(bboxes_deltas,
                                            bboxes_recurr_3d.view(-1,
                                                                  len(
                                                                      self.bbox_weights)),
                                            is_first=False)
        # (bs*n_p, 10)
    ...
    def apply_deltas(self, deltas, boxes, is_first=False):
        ...
        if is_first:
            # for first head as diffusion boxes are BEV, they don't have
            # height or c_z
            ctr_x = boxes_split[0]
            ctr_y = boxes_split[1]

            widths = boxes_split[2]
            lengths = boxes_split[3]
            ...
            # exp here it's required because this boxes are log of size
            # from head 2
        ...
        if len(self.bbox_weights) == 10:
            pred_boxes = torch.cat(
                [pred_ctr_x, pred_ctr_y, pred_ctr_z, pred_w.log(),
                 pred_l.log(),
                 pred_h, deltas_split[6], deltas_split[7],
                 deltas_split[8],
                 deltas_split[9]], dim=-1)  # (bs*n_p, 10)
        else:
            pred_boxes = torch.cat(
                [pred_ctr_x, pred_ctr_y, pred_ctr_z, pred_w.log(),
                 pred_l.log(), pred_h, deltas_split[6], deltas_split[7]],
                dim=-1)  # (bs*n_p, 10)
        # pred_boxes = torch.cat(
    ...
    def img_feats_sampling_bboxes_roi(self, img_feats, bboxes, pooler,
        ...
        #     1, 3)
        # bboxes[..., :3] = (bboxes[..., :3] * pc_range_) + pc_start_  # (
        # n_p, 3)
    ...
    def points_feats_sampling_bboxes_roi(self, points_feats, bboxes, pooler,
        ...
        bboxes (Tensor): (bs, n_p, 5) [x, y, w, h, ry]
        pooler (nn.Module): ROI Extractor
        img_metas (list[dict])
        pc_range (list): [x_min, y_,in, z_min, x_max, y_max, z_max]
        """

        # get the corners of the BEV bboxes
        bbox_bev_corners = []
        for bboxes_sample in bboxes:  # (n_p, 5)
            bbox_bev_corners.append(center_to_corner_box2d(
                bboxes_sample[:, 0:2], bboxes_sample[:, 2:4], bboxes_sample[
                                                              :, 4:5]))
            # (n_p, 4, 2)
        bbox_bev_corners = torch.stack(bbox_bev_corners)
        # (bs, n_p, 4, 2) in world coord

        # convert corners to range [0, 110.4]
        pc_start_ = bboxes.new_tensor(
            [[self.pc_range[0], self.pc_range[1]]])  # (1, 2)
        bbox_bev_corners = bbox_bev_corners - pc_start_  # (bs, n_p, 4, 2)

        # divide by voxel size to get index on BEV
        bbox_bev_corners[..., 0:1] = bbox_bev_corners[..., 0:1] / \
                                     self.voxel_size[0]
        bbox_bev_corners[..., 1:2] = bbox_bev_corners[..., 1:2] / \
                                     self.voxel_size[1]
        # range [0, 1472]

        # bbox2d = torch.cat([bbox_bev_corners[:, :, 0, :], bbox_bev_corners[
        #                                                   :, :, 2, :]], dim=2)
        # (bs, n_p, 4) XYXY
        # because center to corner2d will give 4 corners in
        # x0y0, x0y1, x1y1, x1y0

        minxy = torch.min(bbox_bev_corners, dim=2).values
        # (bs, n_p, 2)
        maxxy = torch.max(bbox_bev_corners, dim=2).values
        # (bs, n_p, 2)
